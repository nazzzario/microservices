package com.mic.gateway.routes;
...
public class Routes {
    ...
    public RouterFunction<ServerResponse> productServiceRoute() {
        return route("product_service")
                .route(path("/api/v1/products"), http("http://localhost:8080"))
    ...
    public RouterFunction<ServerResponse> orderServiceRoute() {
        return route("order_service")
                .route(path("/api/v1/orders"), http("http://localhost:8081"))
    ...
    public RouterFunction<ServerResponse> inventoryServiceRoute() {
        return route("inventory_service")
                .route(path("/api/v1/inventories"), http("http://localhost:8082"))
    ...
    public RouterFunction<ServerResponse> fallbackRoute() {
        return route("fallbackRoute").GET("/fallbackRoute", request -> status(HttpStatus.SERVICE_UNAVAILABLE)
                        .body("Service Unenviable, pleas try again later."))
                .build();
    }
}
